...
<title>Polyglot</title>
	...
	<div class = "row">
		...
		<div class="col-sm-4">
			<div id = "greet">
			<center>Enter Your New Password</center><br>
			</div>
			<div class="col-lg-2">
			</div>
...
<?php
	...
	include "assets/dbconnect.php";
		...
		$conn = dbConnect();
		$account = $conn->prepare("SELECT * FROM tbluser where email='$email'");
		$account->execute();
		$result = $account->fetch(PDO::FETCH_ASSOC);
		$checker =  $result['email'];
		$userforget = $result['userid'];

	
		$conn = dbConnect();
		$accounts = $conn->prepare("INSERT INTO tblforget (userid, keygen)
			VALUES
			('$userforget','$key')");
		$accounts->execute();
		$results = $accounts->fetch(PDO::FETCH_ASSOC);
		
		$account = $conn->prepare("SELECT * FROM tbluser where email='$email'");
		$account->execute();
		$res = $account->fetch(PDO::FETCH_ASSOC);
		$checker =  $res['email'];


		if($email == $checker){
			echo'<center><h4>'.$checker.'</h4></center><br>';
			echo '	<form action = "uppassw.php" method ="post">
					<center><input type="password" class="form-control " placeholder="password" id="answer"  name="password" required><br>
							<input type="password" class="form-control " placeholder="re-enter password" id="answer"  name="password" required><br>
							<input type="hidden" class="form-control "  id="email"  name="email" value= "'.$checker.'" required>
